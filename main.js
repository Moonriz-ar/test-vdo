...
async function main() {
  ...
  if (
    location.hostname !== "vdo.ninja" &&
    location.hostname !== "backup.vdo.ninja" &&
    location.hostname !== "proxy.vdo.ninja" &&
    location.hostname !== "obs.ninja"
  ) {
    errorReport = false;
  ...
  if (
    urlParams.has("cleanoutput") ||
    urlParams.has("clean") ||
    urlParams.has("cleanish")
  ) {
    ...
    if (ChromiumVersion === 65) {
      ...
      if (URLGOTO && URLGOTO.startsWith("https://")) {
        if (URLGOTO === window.location.href) {
          ...
          session.sticky = await confirmAlt(
            "Would you like to load your previous session?\n\nThis will redirect you to:\n\n" +
              URLGOTO,
            true
          );
          if (!session.sticky) {
  ...
  if (
    urlParams.has("avatarimg") ||
    urlParams.has("bgimage") ||
    urlParams.has("bgimg")
  ) {
    ...
    var avatarImg =
      urlParams.get("avatarimg") ||
      urlParams.get("bgimage") ||
      urlParams.get("bgimg") ||
      "./media/avatar1.png";
    if (avatarImg) {
      try {
  ...
  if (
    urlParams.has("avatarimg2") ||
    urlParams.has("bgimage2") ||
    urlParams.has("bgimg2")
  ) {
    // URL or data:base64 image. Becomes local to this viewer only.  This is like &avatar, but slightly different. Just CSS in this case
    var avatarImg2 =
      urlParams.get("avatarimg2") ||
      urlParams.get("bgimage2") ||
      urlParams.get("bgimg2") ||
      "./media/avatar2.png";
    if (avatarImg2) {
      try {
      ...
      try {
        ...
        fallbackImage2.onload = function () {
          document.documentElement.style.setProperty(
            "--video-background-image-talking",
            'url("' + avatarImg2 + '")'
          );
          if (session.meterStyle !== 5) {
            document.documentElement.style.setProperty(
  ...
  if (
    urlParams.has("avatarimg3") ||
    urlParams.has("bgimage3") ||
    urlParams.has("bgimg3")
  ) {
    // URL or data:base64 image. Becomes local to this viewer only.  This is like &avatar, but slightly different. Just CSS in this case
    var avatarImg3 =
      urlParams.get("avatarimg3") ||
      urlParams.get("bgimage3") ||
      urlParams.get("bgimg3") ||
      "./media/avatar3.png";
    if (avatarImg3) {
      try {
      ...
      try {
        ...
        fallbackImage3.onload = function () {
          document.documentElement.style.setProperty(
            "--video-background-image-screaming",
            'url("' + avatarImg3 + '")'
          );
          if (session.meterStyle !== 5) {
            document.documentElement.style.setProperty(
  ...
  if (urlParams.has("background") || urlParams.has("appbg")) {
    ...
    var background =
      urlParams.get("background") || urlParams.get("appbg") || false;
    if (background) {
      try {
  ...
  if (urlParams.has("whip") || urlParams.has("whipview")) {
    session.whipView =
      urlParams.get("whip") || urlParams.get("whipview") || false;
    if (session.whipView) {
      setTimeout(function () {
        whipClient();
      }, 1000);
  ...
  if (
    urlParams.has("whippush") ||
    urlParams.has("whipout") ||
    urlParams.has("pushwhip")
  ) {
    // URL or data:base64 image. Becomes local to this viewer only.  This is like &avatar, but slightly different. Just CSS in this case
    session.whipOutput =
      urlParams.get("whippush") ||
      urlParams.get("whipout") ||
      urlParams.get("pushwhip") ||
      null;
    if (session.whipOutput) {
      try {
        if (session.whipOutput == "twitch") {
          session.whipOutput = "https://g.webrtc.live-video.net:4443/v2/offer";
          query("#publishOutToken input[type='password']").placeholder =
            "Twitch stream token here";
        } else {
  ...
  if (
    urlParams.has("whippushtoken") ||
    urlParams.has("whipouttoken") ||
    urlParams.has("pushwhiptoken")
  ) {
    // URL or data:base64 image. Becomes local to this viewer only.  This is like &avatar, but slightly different. Just CSS in this case
    session.whipOutputToken =
      urlParams.get("whippushtoken") ||
      urlParams.get("whipouttoken") ||
      urlParams.get("pushwhiptoken") ||
      false;
  ...
  if (urlParams.has("whepplaytoken")) {
    // URL or data:base64 image. Becomes local to this viewer only.  This is like &avatar, but slightly different. Just CSS in this case
  ...
  if (urlParams.has("novideobutton") || urlParams.has("nvb")) {
    getById("mutevideobutton").style.setProperty(
      "display",
      "none",
      "important"
    );
  ...
  if (urlParams.has("nospeakerbutton") || urlParams.has("nsb")) {
    getById("mutespeakerbutton").style.setProperty(
      "display",
      "none",
      "important"
    );
  ...
  if (urlParams.has("notmobile")) {
    ...
    if (/CriOS/i.test(navigator.userAgent)) {
      ...
      if (!session.cleanOutput) {
        try {
          ...
          warnUser(
            "Chrome on this device does not support the required technology to use this site.\n\nPlease use Safari instead or update your iOS and browser version."
          );
        }
      }
    }
  } else if (
    /Android|Pixel|webOS|BlackBerry|IEMobile|Opera Mini/i.test(
      navigator.userAgent
    )
  ) {
    ...
    log("MAKE DRAGGABLE");
    delayedStartupFuncs.push([
      makeDraggableElement,
      document.getElementById("subControlButtons"),
    ]);
  ...
  if (urlParams.has("broadcasttransfer") || urlParams.has("bct")) {
    ...
    session.broadcastTransfer =
      urlParams.get("broadcasttransfer") || urlParams.get("bct") || null;
  ...
  if (urlParams.has("queuetransfer") || urlParams.has("qt")) {
    log("Broadcast transfer flag set");
    session.queueTransfer =
      urlParams.get("queuetransfer") || urlParams.get("qt") || null;
  ...
  if (urlParams.has("broadcast") || urlParams.has("bc")) {
    ...
    session.broadcast =
      urlParams.get("broadcast") || urlParams.get("bc") || null;
  ...
  if (urlParams.has("meshcastcode") || urlParams.has("mccode")) {
    session.meshcastCode =
      urlParams.get("meshcastcode") || urlParams.get("mccode") || false;
  }
  ...
  try {
    if (!session.decrypted) {
      filename = window.location.pathname.substring(
      ...
      if (filename.split(".").length == 1) {
        if (filename2.length < 2) {
          ...
          if (history.pushState) {
            var tmpHref = window.location.href.substring(
              0,
              window.location.href.lastIndexOf("/")
            );
  ...
  if (urlParams.has("slotmode") || urlParams.has("slotsmode")) {
    session.slotmode =
      parseInt(urlParams.get("slotmode")) ||
      parseInt(urlParams.get("slotsmode")) ||
      1;
  }
  ...
  if (urlParams.has("showdirector") || urlParams.has("sd")) {
    session.showDirector =
      parseInt(urlParams.get("showdirector")) ||
      parseInt(urlParams.get("sd")) ||
      true; // if 2, video only allowed.  True or 1 will be video + audio allowed.
    // fyi,  true is the same as 1 when == is used, so assert(1==true) is true.
  }

  if (urlParams.has("bitratecutoff") || urlParams.has("bitcut")) {
    session.lowBitrateCutoff =
      parseInt(urlParams.get("bitratecutoff")) ||
      parseInt(urlParams.get("bitcut")) ||
      300; // low bitrate cut off.
  }
  ...
  if (urlParams.has("lowbitratescene") || urlParams.has("cutscene")) {
    session.lowBitrateSceneChange =
      urlParams.get("lowbitratescene") ||
      urlParams.get("cutscene") ||
      "cutscene"; // low bitrate cut off.
  ...
  if (
    urlParams.has("forcelandscape") ||
    urlParams.has("forcedlandscape") ||
    urlParams.has("fl")
  ) {
    ...
    urlParams.has("forceportrait") ||
    urlParams.has("forcedportrait") ||
    urlParams.has("fp")
  ) {
  ...
  document.addEventListener("fullscreenchange", (event) => {
    ...
    if (session.orientation && session.mobile) {
      if (document.fullscreenElement) {
        ...
        getById("fullscreenPageToggle").classList.remove(
  ...
  if (urlParams.has("midi") || urlParams.has("hotkeys")) {
    session.midiHotkeys =
      urlParams.get("midi") || urlParams.get("hotkeys") || 1;
  ...
  if (urlParams.has("midiremote") || urlParams.has("remotemidi")) {
    if (session.director !== false) {
      session.midiRemote =
        parseInt(urlParams.get("midiremote")) ||
        parseInt(urlParams.get("remotemidi")) ||
        4;
    } else {
      session.midiRemote =
        parseInt(urlParams.get("midiremote")) ||
  ...
  if (
    urlParams.has("midipush") ||
    urlParams.has("midiout") ||
    urlParams.has("mo")
  ) {
    session.midiOut =
      parseInt(urlParams.get("midipush")) ||
      parseInt(urlParams.get("midiout")) ||
      parseInt(urlParams.get("mo")) ||
      true;
  }

  if (
    urlParams.has("midipull") ||
    urlParams.has("midiin") ||
    urlParams.has("midin") ||
    urlParams.has("mi")
  ) {
    session.midiIn =
      parseInt(urlParams.get("midipull")) ||
      parseInt(urlParams.get("midiin")) ||
      parseInt(urlParams.get("midin")) ||
      parseInt(urlParams.get("mi")) ||
      true;
  }
  ...
  if (directorLanding) {
    ...
    urlParams.has("webcam") ||
    urlParams.has("wc") ||
    urlParams.has("miconly")
  ) {
    ...
    if (urlParams.has("miconly")) {
      ...
      getById("flipcamerabutton").style.setProperty(
        "display",
        "none",
        "important"
      );
    ...
    session.website =
      urlParams.get("website") || urlParams.get("iframe") || false;
    if (session.website) {
    ...
    if (urlParams.get("screenshare2") || urlParams.get("ss2")) {
      session.screenshare =
        urlParams.get("screenshare2") || urlParams.get("ss2");
    }
  }
  ...
  if (urlParams.has("sstype") || urlParams.has("screensharetype")) {
    ...
    session.screenshareType =
      urlParams.get("sstype") || urlParams.get("screensharetype");
  ...
  if (urlParams.has("selfbrowsersurface")) {
    // exclude
    session.selfBrowserSurface =
      urlParams.get("selfbrowsersurface") || "exclude";
  }
  ...
  if (
    urlParams.has("volumecontrol") ||
    urlParams.has("volumecontrols") ||
    urlParams.has("vc")
  ) {
    if (!(iOS || iPad)) {
  ...
  if (
    urlParams.has("videomute") ||
    urlParams.has("videomuted") ||
    urlParams.has("vm")
  ) {
  ...
  if (urlParams.has("layout")) {
    try {
      session.layout =
        JSON.parse(decodeURIComponent(urlParams.get("layout"))) ||
    ...
    console.warn(
      "Warning: If using &layout with &broadcast, only the director's video will appear in the custom layout, which is likely not intended."
    );
  }
  ...
  if (urlParams.has("layouts")) {
    ...
    try {
      session.layouts =
        JSON.parse(decodeURIComponent(urlParams.get("layouts"))) ||
        JSON.parse(urlParams.get("layouts")) ||
        {};
    } catch (e) {
      try {
  ...
  if (
    urlParams.has("dpi") ||
    urlParams.has("dpr") ||
    urlParams.has("sharper") ||
    urlParams.has("sharpen")
  ) {
    session.devicePixelRatio =
      urlParams.get("dpi") || urlParams.get("dpr") || 2.0;
  ...
  if (
    urlParams.has("speakermute") ||
    urlParams.has("speakermuted") ||
    urlParams.has("mutespeaker") ||
    urlParams.has("sm") ||
    urlParams.has("ms")
  ) {
    var checkState =
      urlParams.get("speakermute") ||
      urlParams.get("speakermuted") ||
      urlParams.get("mutespeaker") ||
      urlParams.get("sm") ||
      urlParams.get("ms") ||
      true;
  ...
  if (
    urlParams.has("chatbutton") ||
    urlParams.has("chat") ||
    urlParams.has("cb")
  ) {
    session.chatbutton =
      urlParams.get("chatbutton") ||
      urlParams.get("chat") ||
      urlParams.get("cb") ||
      null;
  ...
  if (
    urlParams.has("portrait") ||
    urlParams.has("916") ||
    urlParams.has("vertical")
  ) {
  ...
  if (urlParams.has("aspectratio") || urlParams.has("ar")) {
    ...
    session.forceAspectRatio =
      urlParams.get("aspectratio") || urlParams.get("ar") || false;
    if (session.forceAspectRatio) {
      if (
        session.forceAspectRatio == "portrait" ||
        session.forceAspectRatio == "vertical"
      ) {
        ...
        session.forceAspectRatio =
          parseFloat(session.forceAspectRatio) || false;
      }
    }
  }
  if (urlParams.has("screenshareaspectratio") || urlParams.has("ssar")) {
    // capture aspect ratio
    session.forceScreenShareAspectRatio =
      urlParams.get("screenshareaspectratio") ||
      urlParams.get("ssar") ||
      16.0 / 9.0;
    if (session.forceScreenShareAspectRatio) {
      if (
        session.forceScreenShareAspectRatio == "portrait" ||
        session.forceScreenShareAspectRatio == "vertical"
      ) {
        ...
        session.forceScreenShareAspectRatio =
          parseFloat(session.forceScreenShareAspectRatio) || false;
      }
    }
  }
  ...
  if (urlParams.has("record")) {
    if (!session.cleanOutput) {
      if (SafariVersion && !MediaRecorder) {
        if (macOS) {
          warnUser(
            "Your browser may not support local media recording.\n\nTry Chrome instead if on macOS."
          );
        } else {
          warnUser(
            "Your browser or device may not support local media recording.\n\nSafari sometimes allows the feature to be enabled via its experimental settings."
          );
        }
      } else if (SafariVersion) {
        if (macOS) {
          warnUser(
            "It is recommended to use Chrome instead of Safari if doing local media recordings."
          );
        } else {
          warnUser(
            "Local media recordings are an experimental feature on Apple devices.\n\nPlease at least test it out a few times first."
          );
        }
      }
    }
  ...
  if (session.record === false) {
    ...
    try {
      document
        .querySelectorAll('[data-action-type^="record"]')
        .forEach((ele) => {
          ele.remove();
          delete ele;
        });
  ...
  if (urlParams.has("recordcodec") || urlParams.has("rc")) {
    session.recordingVideoCodec =
      urlParams.get("recordcodec") || urlParams.get("rc") || false;
  }
  ...
  if (
    urlParams.has("nopush") ||
    urlParams.has("noseed") ||
    urlParams.has("viewonly") ||
    urlParams.has("viewmode")
  ) {
  ...
  if (session.dataMode) {
    if (
      !(session.meshcast || session.whipOutput !== false || session.screenshare)
    ) {
      session.videoDevice = 0;
  ...
  if (urlParams.has("hiddenscenebitrate")) {
    session.hiddenSceneViewBitrate =
      parseInt(urlParams.get("hiddenscenebitrate")) || 0;
  }
  ...
  if (urlParams.has("scenetype") || urlParams.has("type")) {
    session.sceneType =
      parseInt(urlParams.get("scenetype")) ||
      parseInt(urlParams.get("type")) ||
      false;
  }
  ...
  if (
    urlParams.has("transcript") ||
    urlParams.has("transcribe") ||
    urlParams.has("trans")
  ) {
    session.transcript =
      urlParams.get("transcript") ||
      urlParams.get("transcribe") ||
      urlParams.get("trans") ||
      "en-US";
  }
  ...
  if (
    urlParams.has("cc") ||
    urlParams.has("closedcaptions") ||
    urlParams.has("captions")
  ) {
  ...
  if (
    urlParams.has("prompt") ||
    urlParams.has("validate") ||
    urlParams.has("approve")
  ) {
  ...
  if (urlParams.has("js")) {
    ...
    console.warn(
      "Third-party Javascript has been injected into the code. Security cannot be ensured."
    );
  ...
  if (
    urlParams.has("base64js") ||
    urlParams.has("b64js") ||
    urlParams.has("jsbase64") ||
    urlParams.has("jsb64")
  ) {
    try {
      var base64js =
  ...
  if (
    urlParams.has("base64css") ||
    urlParams.has("b64css") ||
    urlParams.has("cssbase64") ||
    urlParams.has("cssb64")
  ) {
    try {
      var base64Css =
        urlParams.get("base64css") ||
        urlParams.get("b64css") ||
        urlParams.get("cssbase64") ||
        urlParams.get("cssb64");
  ...
  if (
    ...
    if (!session.password) {
      window.focus();
      session.password = await promptAlt(
        getTranslation("enter-password"),
        true,
        true
      );
      if (session.password) {
    ...
    urlParams.has("nopassword") ||
    urlParams.has("nopass") ||
    urlParams.has("nopw") ||
    urlParams.has("p0")
  ) {
    session.password = false;
  ...
  if (urlParams.has("hash") || urlParams.has("crc") || urlParams.has("check")) {
    ...
    var hash_input =
      urlParams.get("hash") || urlParams.get("crc") || urlParams.get("check");
    if (session.password === false) {
      window.focus();
      session.password = await promptAlt(
        getTranslation("enter-password-2"),
        true,
        true
      );
      session.password = sanitizePassword(session.password);
    ...
    generateHash(session.password + session.salt, 6)
      .then(function (hash) {
        ...
        log("hash is " + hash);
        if (hash.substring(0, 4) !== hash_input) {
          // hash crc checks are just first 4 characters.
  ...
  if (
    urlParams.has("showlabels") ||
    urlParams.has("showlabel") ||
    urlParams.has("sl")
  ) {
    session.showlabels =
      urlParams.get("showlabels") ||
      urlParams.get("showlabel") ||
      urlParams.get("sl") ||
      "";
    session.showlabels = sanitizeLabel(
      session.showlabels.replace(/[\W]+/g, "_").replace(/_+/g, "_")
    );
    //session.style = 6;
  ...
  if (
    urlParams.has("sizelabel") ||
    urlParams.has("labelsize") ||
    urlParams.has("fontsize")
  ) {
    session.labelsize =
      urlParams.get("sizelabel") ||
      urlParams.get("labelsize") ||
      urlParams.get("fontsize") ||
      100;
  ...
  if (urlParams.has("label") || urlParams.has("l")) {
    ...
    if (session.label == null || session.label.length == 0) {
      window.focus();
      session.label = await promptAlt(
        getTranslation("enter-display-name"),
        true
      );
    } else {
    ...
    urlParams.has("defaultlabel") ||
    urlParams.has("labelsuggestion") ||
    urlParams.has("ls")
  ) {
    session.label =
      urlParams.get("defaultlabel") ||
      urlParams.get("labelsuggestion") ||
      urlParams.get("ls") ||
      null;
    var updateURLAsNeed = true;
    window.focus();
  ...
  if (
    urlParams.has("stereo") ||
    urlParams.has("s") ||
    urlParams.has("proaudio")
  ) {
    ...
    log("STEREO ENABLED");
    session.stereo =
      urlParams.get("stereo") ||
      urlParams.get("s") ||
      urlParams.get("proaudio");
  ...
  if (
    urlParams.has("screensharestereo") ||
    urlParams.has("sss") ||
    urlParams.has("ssproaudio")
  ) {
    // both peers need this enabled for HD stereo to be on. If just pub, you get no echo/noise cancellation. if just viewer, you get high bitrate mono
    log("screenshare stereo  ENABLED");
    session.screenshareStereo =
      urlParams.get("screensharestereo") ||
      urlParams.get("sss") ||
      urlParams.get("ssproaudio");
  ...
  if (urlParams.has("pie")) {
    ...
    if (session.customWSS && session.customWSS !== true) {
      session.wss =
        "wss://free3.piesocket.com/v3/1?api_key=" + session.customWSS; // if URL param is set, it will use the API key.
    }
  }
  ...
  if (
    urlParams.has("channelcount") ||
    urlParams.has("ac") ||
    urlParams.has("inputchannels")
  ) {
    // if updates to this, see also function toggleMonoStereoMic()
    session.audioInputChannels =
      urlParams.get("channelcount") ||
      urlParams.get("ac") ||
      urlParams.get("inputchannels") ||
      0;
  ...
  if (
    ...
    session.echoCancellation =
      urlParams.get("echocancellation") ||
      urlParams.get("aec") ||
      urlParams.get("ec");
  ...
  if (
    urlParams.has("autogain") ||
    urlParams.has("ag") ||
    urlParams.has("agc")
  ) {
    session.autoGainControl =
      urlParams.get("autogain") || urlParams.get("ag") || urlParams.get("agc");
  ...
  if (
    urlParams.has("screenshareaec") ||
    urlParams.has("ssec") ||
    urlParams.has("ssaec")
  ) {
    session.screenshareAEC =
      urlParams.get("screenshareaec") ||
      urlParams.get("ssec") ||
      urlParams.get("ssaec");
  ...
  if (
    urlParams.has("screenshareautogain") ||
    urlParams.has("ssag") ||
    urlParams.has("ssagc")
  ) {
    session.screenshareAutogain =
      urlParams.get("screenshareautogain") ||
      urlParams.get("ssag") ||
      urlParams.get("ssagc");
  ...
  if (urlParams.has("screensharedenoise") || urlParams.has("ssdn")) {
    session.screenshareDenoise =
      urlParams.get("screensharedenoise") || urlParams.get("ssdn");
  ...
  if (
    urlParams.has("roombitrate") ||
    urlParams.has("roomvideobitrate") ||
    urlParams.has("rbr")
  ) {
    log("Room BITRATE SET");
    session.roombitrate =
      urlParams.get("roombitrate") ||
      urlParams.get("rbr") ||
      urlParams.get("roomvideobitrate");
  ...
  if (urlParams.has("outboundaudiobitrate") || urlParams.has("oab")) {
    session.outboundAudioBitrate =
      parseInt(urlParams.get("outboundaudiobitrate")) ||
      parseInt(urlParams.get("oab")) ||
      false;
  }
  if (
    urlParams.has("outboundvideobitrate") ||
    urlParams.has("outboundbitrate") ||
    urlParams.has("ovb")
  ) {
    session.outboundVideoBitrate =
      parseInt(urlParams.get("outboundvideobitrate")) ||
      parseInt(urlParams.get("outboundbitrate")) ||
      parseInt(urlParams.get("ovb")) ||
      false;
  }
  ...
  if (
    urlParams.has("webpquality") ||
    urlParams.has("webpq") ||
    urlParams.has("wq")
  ) {
    session.webPquality =
      parseInt(urlParams.get("webpquality")) ||
      parseInt(urlParams.get("webpq")) ||
      parseInt(urlParams.get("wq")) ||
      4;
  }
  ...
  if (
    ...
    // the streams we want to view; if set, but let blank, we will request no streams to watch.
    session.view =
      urlParams.get("streamid") ||
      urlParams.get("view") ||
      urlParams.get("v") ||
      urlParams.get("pull") ||
      null; // this value can be comma seperated for multiple streams to pull
  ...
  if (urlParams.has("fakeguests") || urlParams.has("fakefeeds")) {
    var total =
      parseInt(urlParams.get("fakeguests")) ||
      parseInt(urlParams.get("fakefeeds")) ||
      4;
    session.fakeFeeds = [];
  ...
  if (
    urlParams.has("directoronly") ||
    urlParams.has("directorsonly") ||
    urlParams.has("do")
  ) {
  ...
  if (
    urlParams.has("ruler") ||
    urlParams.has("grid") ||
    urlParams.has("thirds")
  ) {
    session.fullscreen = true;
    if (!session.manual) {
      session.manual = false;
    }
    session.ruleOfThirds =
      urlParams.get("ruler") ||
      urlParams.get("grid") ||
      urlParams.get("thirds") ||
      "./media/thirds.svg";
  ...
  if (urlParams.has("minipreviewoffset") || urlParams.has("mpo")) {
    ...
    session.leftMiniPreview =
      urlParams.get("minipreviewoffset") || urlParams.get("mpo") || 0;
  ...
  if (urlParams.has("minroombitrate") || urlParams.has("mrb")) {
    session.minimumRoomBitrate =
      urlParams.get("minroombitrate") || urlParams.get("mrb") || false;
  ...
  if (
    urlParams.has("latency") ||
    urlParams.has("al") ||
    urlParams.has("audiolatency")
  ) {
    log("latency  ENABLED");
    session.audioLatency =
      urlParams.get("latency") ||
      urlParams.get("al") ||
      urlParams.get("audiolatency");
    session.audioLatency = parseInt(session.audioLatency) || 0;
    session.disableWebAudio = false;
  }

  if (
    urlParams.has("micdelay") ||
    urlParams.has("delay") ||
    urlParams.has("md")
  ) {
    log("audio gain  ENABLED");
    session.micDelay =
      urlParams.get("micdelay") ||
      urlParams.get("delay") ||
      urlParams.get("md") ||
      0;
  ...
  if (
    urlParams.has("audiogain") ||
    urlParams.has("gain") ||
    urlParams.has("g")
  ) {
    log("audio gain  ENABLED");
    session.audioGain =
      urlParams.get("audiogain") || urlParams.get("gain") || urlParams.get("g");
  ...
  if (
    urlParams.has("lowcut") ||
    urlParams.has("lc") ||
    urlParams.has("higpass")
  ) {
    session.lowcut =
      urlParams.get("lowcut") ||
      urlParams.get("lc") ||
      urlParams.get("higpass") ||
      100;
  ...
  if (
    urlParams.has("keyframeinterval") ||
    urlParams.has("keyframerate") ||
    urlParams.has("keyframe") ||
    urlParams.has("fki")
  ) {
    log("keyframeRate ENABLED");
    session.keyframeRate =
      parseInt(
        urlParams.get("keyframeinterval") ||
          urlParams.get("keyframerate") ||
          urlParams.get("keyframe") ||
          urlParams.get("fki")
      ) || 0;
  }

  if (
    urlParams.has("obsoff") ||
    urlParams.has("oo") ||
    urlParams.has("disableobs")
  ) {
  ...
  if (
    urlParams.has("hidecodirectors") ||
    urlParams.has("hidecodirector") ||
    urlParams.has("hidedirector") ||
    urlParams.has("hidedirectors") ||
    urlParams.has("hd")
  ) {
    document
      .querySelector(":root")
      .style.setProperty("--show-codirectors", "none", "important");
    session.hideDirector = true;
  }

  if (
    urlParams.has("pptcontrols") ||
    urlParams.has("slides") ||
    urlParams.has("ppt") ||
    urlParams.has("powerpoint")
  ) {
  ...
  if (
    urlParams.has("obscontrols") ||
    urlParams.has("remoteobs") ||
    urlParams.has("obsremote") ||
    urlParams.has("obs") ||
    urlParams.has("controlobs")
  ) {
    session.obsControls =
      urlParams.get("obscontrols") ||
      urlParams.get("remoteobs") ||
      urlParams.get("obsremote") ||
      urlParams.get("obs") ||
      urlParams.get("controlobs");
  ...
  if (
    urlParams.has("tallyoff") ||
    urlParams.has("notally") ||
    urlParams.has("disabletally") ||
    urlParams.has("to")
  ) {
    log("Tally Light off");
    getById("obsState").style.setProperty("display", "none", "important");
  ...
  if (urlParams.has("chroma")) {
    log("Chroma ENABLED");
    getById("main").style.backgroundColor =
      "#" + (urlParams.get("chroma") || "0F0");
  ...
  if (urlParams.has("rounded") || urlParams.has("round")) {
    try {
      session.borderRadius =
        urlParams.get("rounded") || urlParams.get("round") || 50;
      session.borderRadius = parseInt(session.borderRadius);
      document
        .querySelector(":root")
        .style.setProperty("--video-rounded", session.borderRadius + "px");
    } catch (e) {
      errorlog("variable css failed");
    }
  }
  ...
  if (urlParams.has("border")) {
    try {
      ...
      document
        .querySelector(":root")
        .style.setProperty("--video-border-color", "#000");
      document
        .querySelector(":root")
        .style.setProperty("--video-border", videoBorder);
    } catch (e) {
      errorlog("variable css failed");
    }
  }
  ...
  if (urlParams.has("bordercolor")) {
    try {
      ...
      document
        .querySelector(":root")
        .style.setProperty("--video-border-color", session.borderColor);
    } catch (e) {
      errorlog("variable css failed");
    }
  }
  ...
  if (urlParams.has("color")) {
    session.colorVideosBackground =
      urlParams.get("color") || session.borderColor || "#000";
  }
  ...
  try {
    if (urlParams.has("darkmode") || urlParams.has("nightmode")) {
      session.darkmode =
        urlParams.get("darkmode") || urlParams.get("nightmode") || null;
      ...
      session.darkmode = getComputedStyle(document.querySelector(":root"))
  ...
  if (
    urlParams.has("videodevice") ||
    urlParams.has("vdevice") ||
    urlParams.has("vd") ||
    ...
    session.videoDevice =
      urlParams.get("videodevice") ||
      urlParams.get("vdevice") ||
      urlParams.get("vd") ||
    ...
    if (session.videoDevice === null) {
      ...
      session.videoDevice = session.videoDevice
        .toLowerCase()
        .replace(/[\W]+/g, "_");
    }
  ...
  // audioDevice
  if (
    urlParams.has("audiodevice") ||
    urlParams.has("adevice") ||
    urlParams.has("ad") ||
    urlParams.has("device") ||
    urlParams.has("d")
  ) {
    session.audioDevice =
      urlParams.get("audiodevice") ||
      urlParams.get("adevice") ||
      urlParams.get("ad") ||
      urlParams.get("device") ||
      urlParams.get("d");
    ...
    if (session.audioDevice === null) {
      ...
      session.audioDevice = session.audioDevice
        .toLowerCase()
  ...
  if (session.dataMode) {
    ...
    if (session.consent) {
      setTimeout(function () {
        warnUser(
          "⚠ Privacy warning: The director of this room can remotely switch your camera or microphone without permission.",
          8000
        );
      }, 1500);
    }
  ...
  if (
    urlParams.has("noiframe") ||
    urlParams.has("noiframes") ||
    urlParams.has("nif") ||
    urlParams.has("nowebsite")
  ) {
    session.noiframe =
      urlParams.get("noiframe") ||
      urlParams.get("noiframes") ||
      urlParams.get("nif") ||
      urlParams.get("nowebsite");
  ...
  if (
    urlParams.has("novideo") ||
    urlParams.has("nv") ||
    urlParams.has("hidevideo") ||
    urlParams.has("showonly")
  ) {
    session.novideo =
      urlParams.get("novideo") ||
      urlParams.get("nv") ||
      urlParams.get("hidevideo") ||
      urlParams.get("showonly");
  ...
  if (
    urlParams.has("noaudio") ||
    urlParams.has("na") ||
    urlParams.has("hideaudio")
  ) {
    session.noaudio =
      urlParams.get("noaudio") ||
      urlParams.get("na") ||
      urlParams.get("hideaudio");
  ...
  if (urlParams.has("maindirectorpassword") || urlParams.has("maindirpass")) {
    session.mainDirectorPassword =
      urlParams.get("maindirectorpassword") ||
      urlParams.get("maindirpass") ||
      false;
    if (!session.mainDirectorPassword) {
      window.focus();
      session.mainDirectorPassword = await promptAlt(
        getTranslation("director-password"),
        true,
        true
      );
      if (session.mainDirectorPassword) {
        session.mainDirectorPassword = session.mainDirectorPassword.trim();
        session.mainDirectorPassword = decodeURIComponent(
          session.mainDirectorPassword
        );
      }
    }
  ...
  if (
    urlParams.has("groupview") ||
    urlParams.has("viewgroup") ||
    urlParams.has("gv")
  ) {
    session.groupView =
      urlParams.get("groupview") ||
      urlParams.get("viewgroup") ||
      urlParams.get("gv") ||
      "";
  ...
  if (
    urlParams.has("sensors") ||
    urlParams.has("sensor") ||
    urlParams.has("gyro") ||
    urlParams.has("gyros") ||
    urlParams.has("accelerometer")
  ) {
    session.sensorData =
      urlParams.get("sensors") ||
      urlParams.get("sensor") ||
      urlParams.get("gyro") ||
      urlParams.get("gyros") ||
      urlParams.get("accelerometer") ||
      30;
    session.sensorData = parseInt(session.sensorData);
  }
  if (
    urlParams.has("sensorfilter") ||
    urlParams.has("sensorsfilter") ||
    urlParams.has("filtersensor") ||
    urlParams.has("filtersensors")
  ) {
    session.sensorDataFilter =
      urlParams.get("sensorfilter") ||
      urlParams.get("sensorsfilter") ||
      urlParams.get("filtersensor") ||
      urlParams.get("filtersensors") ||
      "";
  ...
  if (
    urlParams.has("contenthint") ||
    urlParams.has("contenttype") ||
    urlParams.has("content") ||
    urlParams.has("hint")
  ) {
    session.contentHint =
      urlParams.get("contenthint") ||
      urlParams.get("contenttype") ||
  ...
  if (
    urlParams.has("audiocontenthint") ||
    urlParams.has("audiocontenttype") ||
    urlParams.has("audiocontent") ||
    urlParams.has("audiohint")
  ) {
    session.audioContentHint =
      urlParams.get("audiocontenthint") ||
      urlParams.get("audiocontenttype") ||
      urlParams.get("audiocontent") ||
      urlParams.get("audiohint") ||
      "music";
  }

  if (
    urlParams.has("screensharecontenthint") ||
    urlParams.has("sscontenthint") ||
    urlParams.has("screensharecontenttype") ||
    urlParams.has("sscontent") ||
    urlParams.has("sshint")
  ) {
    session.screenshareContentHint =
      urlParams.get("screensharecontenthint") ||
      urlParams.get("sscontenthint") ||
      urlParams.get("screensharecontenttype") ||
      urlParams.get("sscontent") ||
      urlParams.get("sshint") ||
      "detail";
  }

  if (
    urlParams.has("codec") ||
    urlParams.has("codecs") ||
    urlParams.has("videocodec")
  ) {
    log("codecs CHANGED");
    session.codecs =
      urlParams.get("codec") ||
      urlParams.get("codecs") ||
      urlParams.get("videocodec") ||
      false;
    if (session.codecs) {
    ...
    session.codec = "vp8";
    warnlog(
      "Defaulting to VP8 manually, as H264 with remote iOS devices is not supported"
    );
  }
  ...
  if (urlParams.has("scenelinkbitrate")) {
    ...
    if (session.bitrateGroupFlag) {
      session.bitrateGroupFlag =
        "&totalbitrate=" + parseInt(session.bitrateGroupFlag);
    }
  }
  ...
  if (urlParams.has("scale")) {
    ...
    if (urlParams.has("viewwidth") || urlParams.has("vw")) {
      session.viewwidth =
        urlParams.get("viewwidth") || urlParams.get("vw") || false;
    ...
    if (urlParams.has("viewheight") || urlParams.has("vh")) {
      session.viewheight =
        urlParams.get("viewheight") || urlParams.get("vh") || false;
      session.dynamicScale = false; // default true
  ...
  if (urlParams.has("mcscale") || urlParams.has("meshcastscale")) {
    session.meshcastScale =
      parseFloat(urlParams.get("mcscale")) ||
      parseFloat(urlParams.get("meshcastscale")) ||
      100;
  }
  ...
  if (
    ...
    getById("testtone").parentNode.insertBefore(
      addtone,
      getById("testtone").nextSibling
    );
    var addtone = createAudioElement();
  ...
  if (
    urlParams.has("videobitrate") ||
    urlParams.has("bitrate") ||
    urlParams.has("vb")
  ) {
    session.bitrate =
      urlParams.get("videobitrate") ||
  ...
  if (
    urlParams.has("maxvideobitrate") ||
    urlParams.has("maxbitrate") ||
    urlParams.has("mvb")
  ) {
    session.maxvideobitrate =
      urlParams.get("maxvideobitrate") ||
      urlParams.get("maxbitrate") ||
      urlParams.get("mvb");
  ...
  if (
    urlParams.has("totalroombitrate") ||
    urlParams.has("totalroomvideobitrate") ||
    urlParams.has("trb") ||
    urlParams.has("totalbitrate") ||
    urlParams.has("tb")
  ) {
    session.totalRoomBitrate =
      urlParams.get("totalroombitrate") ||
      urlParams.get("totalroomvideobitrate") ||
      urlParams.get("trb") ||
      urlParams.get("totalbitrate") ||
      urlParams.get("tb") ||
      0;
  ...
  if (session.totalRoomBitrate === false) {
    session.totalRoomBitrate =
      session.bitrate || session.totalRoomBitrate_default; // sneaky sneaky
  } else {
  ...
  if (session.totalRoomBitrate_default > 4000) {
    getById("trbSettingInput").max = Math.ceil(
      session.totalRoomBitrate_default
    );
  }

  if (
    urlParams.has("maxtotalscenebitrate") ||
    urlParams.has("totalscenebitrate") ||
    urlParams.has("mtsb") ||
    urlParams.has("tsb") ||
    urlParams.has("totalbitrate") ||
    urlParams.has("tb")
  ) {
    session.totalSceneBitrate =
      urlParams.get("maxtotalscenebitrate") ||
      urlParams.get("totalscenebitrate") ||
      urlParams.get("mtsb") ||
      urlParams.get("tsb") ||
      urlParams.get("totalbitrate") ||
      urlParams.get("tb") ||
  ...
  if (urlParams.has("limittotalbitrate") || urlParams.has("ltb")) {
    session.limitTotalBitrate =
      urlParams.get("limittotalbitrate") || urlParams.get("ltb") || 2500;
    session.limitTotalBitrate = parseInt(session.limitTotalBitrate);
  }

  if (
    urlParams.has("mcb") ||
    urlParams.has("mcbitrate") ||
    urlParams.has("meshcastbitrate")
  ) {
    session.meshcastBitrate =
      urlParams.get("mcb") ||
      urlParams.get("mcbitrate") ||
      urlParams.get("meshcastbitrate") ||
      2500;
    session.meshcastBitrate = parseInt(session.meshcastBitrate);
  }

  if (urlParams.has("mcscreensharebitrate") || urlParams.has("mcssbitrate")) {
    session.meshcastScreenShareBitrate =
      urlParams.get("mcscreensharebitrate") ||
      urlParams.get("mcssbitrate") ||
      2500;
    session.meshcastScreenShareBitrate = parseInt(
      session.meshcastScreenShareBitrate
    );
  }

  if (urlParams.has("mcscreensharecodec") || urlParams.has("mcsscodec")) {
    session.meshcastScreenShareCodec =
      urlParams.get("mcscreensharecodec") ||
      urlParams.get("mcsscodec") ||
      false;
  }
  if (session.meshcastScreenShareCodec) {
    session.meshcastScreenShareCodec =
      session.meshcastScreenShareCodec.toLowerCase();
  }

  if (
    urlParams.has("mcab") ||
    urlParams.has("mcaudiobitrate") ||
    urlParams.has("meshcastab") ||
    urlParams.has("meshcastaudiobitrate ")
  ) {
    session.meshcastAudioBitrate =
      urlParams.get("mcab") ||
      urlParams.get("mcaudiobitrate") ||
      urlParams.get("meshcastab") ||
      urlParams.get("meshcastaudiobitrate ") ||
      32;
    session.meshcastAudioBitrate = parseInt(session.meshcastAudioBitrate);
  }

  if (urlParams.has("mccodec") || urlParams.has("meshcastcodec")) {
    session.meshcastCodec =
      urlParams.get("mccodec") || urlParams.get("meshcastcodec") || false;
  }
  ...
  if (urlParams.has("whipoutcodec") || urlParams.has("woc")) {
    session.whipOutCodec =
      urlParams.get("whipoutcodec") || urlParams.get("woc") || false;
  ...
  if (urlParams.has("whipoutaudiobitrate") || urlParams.has("woab")) {
    session.whipOutAudioBitrate =
      urlParams.get("whipoutaudiobitrate") || urlParams.get("woab") || false;
  ...
  if (urlParams.has("whipoutvideobitrate") || urlParams.has("wovb")) {
    session.whipOutVideoBitrate =
      urlParams.get("whipoutvideobitrate") || urlParams.get("wovb") || false;
  ...
  if (urlParams.has("sink")) {
    ...
    urlParams.has("outputdevice") ||
    urlParams.has("od") ||
    urlParams.has("audiooutput")
  ) {
    session.outputDevice =
      urlParams.get("outputdevice") ||
      urlParams.get("od") ||
      urlParams.get("audiooutput") ||
      null;

    if (session.outputDevice) {
      session.outputDevice = session.outputDevice
        .toLowerCase()
        .replace(/[\W]+/g, "_");
    } else {
  ...
  getById("credits").innerHTML =
    "Version: " + session.version + " - " + getById("credits").innerHTML;
  ...
  if (
    urlParams.has("hideheader") ||
    urlParams.has("noheader") ||
    urlParams.has("hh")
  ) {
  ...
  if (urlParams.has("postinterval")) {
    ...
    session.postInterval =
      urlParams.get("postinterval") || session.postInterval;
  ...
  if (urlParams.has("postimage")) {
    var postURL =
      decodeURIComponent(urlParams.get("postimage")) || session.postURL; // default will post to https://temp.vdo.ninja/images/STREAMIDHERE.jpg
    setInterval(
      function (postURL) {
        try {
  ...
  if (urlParams.has("maxpublishers") || urlParams.has("mp")) {
    session.maxpublishers =
      urlParams.get("maxpublishers") || urlParams.get("mp");
  ...
  if (urlParams.has("maxconnections") || urlParams.has("mc")) {
    session.maxconnections =
      urlParams.get("maxconnections") || urlParams.get("maxconnections");
  ...
  if (
    urlParams.has("frameRate") ||
    urlParams.has("fr") ||
    urlParams.has("fps")
  ) {
    session.frameRate =
      urlParams.get("frameRate") || urlParams.get("fr") || urlParams.get("fps");
  ...
  if (
    urlParams.has("maxframerate") ||
    urlParams.has("mfr") ||
    urlParams.has("mfps")
  ) {
    session.maxframeRate =
      urlParams.get("maxframerate") ||
      urlParams.get("mfr") ||
      urlParams.get("mfps");
  ...
  if (urlParams.has("buffer") || urlParams.has("buffer2")) {
    ...
    if (ChromiumVersion > 50 && ChromiumVersion < 78) {
      ...
      session.buffer =
        parseFloat(urlParams.get("buffer")) ||
        parseFloat(urlParams.get("buffer2")) ||
        0;
  ...
  if (
    urlParams.has("viewereffect") ||
    urlParams.has("viewereffects") ||
    urlParams.has("ve")
  ) {
    session.viewereffects =
      parseInt(urlParams.get("viewereffect")) ||
      parseInt(urlParams.get("ve")) ||
      false;
  }

  if (
    urlParams.has("activespeaker") ||
    urlParams.has("speakerview") ||
    urlParams.has("sas")
  ) {
    session.activeSpeaker =
      urlParams.get("activespeaker") ||
      urlParams.get("speakerview") ||
      urlParams.get("sas") ||
      1;
    ...
    urlParams.has("noisegate") ||
    urlParams.has("gating") ||
    urlParams.has("gate") ||
    urlParams.has("ng")
  ) {
    session.quietOthers =
      urlParams.get("noisegate") ||
      urlParams.get("gating") ||
      urlParams.get("gate") ||
      urlParams.get("ng") ||
      1;
  ...
  if (urlParams.has("fadein")) {
    ...
    if (urlParams.get("fadein") || 0) {
      try {
        ...
        document
          .querySelector(":root")
          .style.setProperty("--fadein-speed", fadeinspeed);
      } catch (e) {
        errorlog("variable css failed");
      }
    } else {
  ...
  if (urlParams.has("animated") || urlParams.has("animate")) {
    session.animatedMoves =
      urlParams.get("animated") || urlParams.get("animate");
  ...
  if (urlParams.has("meter") || urlParams.has("meterstyle")) {
    ...
    session.meterStyle =
      urlParams.get("meter") || urlParams.get("meterstyle") || 1;
  ...
  if (session.meterStyle == 5) {
    document.documentElement.style.setProperty(
      "--video-background-image-size-talking",
      "auto 35%"
    );
    document.documentElement.style.setProperty(
      "--video-background-image-size-screaming",
      "auto 45%"
    );
  }
  ...
  if (urlParams.has("samplerate") || urlParams.has("sr")) {
    session.sampleRate =
      parseInt(urlParams.get("samplerate")) ||
      parseInt(urlParams.get("samplerate")) ||
      48000;
    if (session.audioCtx) {
  ...
  if (urlParams.has("micsamplerate") || urlParams.has("msr")) {
    session.micSampleRate =
      parseInt(urlParams.get("micsamplerate")) ||
      parseInt(urlParams.get("msr")) ||
      48000;
  }
  ...
  if (
    urlParams.has("privacy") ||
    urlParams.has("private") ||
    urlParams.has("relay")
  ) {
    // please only use if you are also using your own TURN service.
    session.privacy =
      urlParams.get("privacy") ||
  ...
  if (urlParams.has("postapi") || urlParams.has("posturl")) {
    session.postApi =
      urlParams.get("postapi") || urlParams.get("posturl") || false; // ie: &postapi=https%3A%2F%2Fwebhook.site%2Fb190f5bf-e4f8-454a-bd51-78b5807df9c1
    if (session.postApi) {
      try {
  ...
  if (
    urlParams.has("push") ||
    urlParams.has("id") ||
    urlParams.has("permaid")
  ) {
    session.permaid =
      urlParams.get("push") || urlParams.get("id") || urlParams.get("permaid");
    ...
    if (session.permaid) {
      ...
      session.streamID =
        sanitizeStreamID(getStorage("permaid")) || session.streamID;
    ...
    if (session.director) {
      ...
      if (session.webcamonly == true) {
        ...
        getById("head1").innerHTML =
          '<span style="color:#CCC;" data-translate="please-accept-permissions">- Please accept any camera permissions</span>';
      } else {
        getById("head1").innerHTML =
          '<br /><span style="color:#CCC" data-translate="please-select-which-to-share">- Please select which you wish to share</span>';
      }
  ...
  if (
    ...
    if (session.director) {
      if (session.director !== session.roomid) {
        if (!session.cleanOutput) {
          warnUser(
            "Conflicting director and room values were provided.\n\n Check your URL parameters; there should be only &director OR &room",
            5000
          );
        }
      }
  ...
  if (urlParams.has("effects") || urlParams.has("effect")) {
    session.effect =
      urlParams.get("effects") || urlParams.get("effect") || null;
  ...
  if (window.FaceDetector !== undefined) {
    document.querySelectorAll(".facetracker").forEach((ele) => {
      ...
      ele.title =
        "Will slowly pan, tilt, and zoom in on the first face detected";
    });
  }
  ...
  if (urlParams.has("effectvalue") || urlParams.has("ev")) {
    session.effectValue =
      parseInt(urlParams.get("effectvalue")) ||
      parseInt(urlParams.get("ev")) ||
      0;
  ...
  if (
    ...
    session.introOnClean &&
    session.scene === false &&
    (session.permaid !== false || session.roomid !== false)
  ) {
    getById("container-2").className = "column columnfade hidden"; // Hide screen share
    getById("container-3").classList.add("skip-animation");
    getById("container-3").classList.remove("pointer");
  ...
  if (
    urlParams.has("hidescreenshare") ||
    urlParams.has("hidess") ||
    urlParams.has("sshide") ||
    urlParams.has("screensharehide")
  ) {
  ...
  if (
    urlParams.has("sspaused") ||
    urlParams.has("sspause") ||
    urlParams.has("ssp")
  ) {
    // this way I don't need to remember what it's called. I can just guess. :D
  ...
  if (urlParams.has("zoomedbitrate") || urlParams.has("zb")) {
    // this way I don't need to remember what it's called. I can just guess. :D
    session.zoomedBitrate =
      urlParams.get("zoomedbitrate") || urlParams.get("zb") || 2500;
  ...
  if (urlParams.has("screenshareid") || urlParams.has("ssid")) {
    if (urlParams.get("screenshareid") || urlParams.get("ssid")) {
      session.screenshareid =
        urlParams.get("screenshareid") || urlParams.get("ssid");
  ...
  if (
    urlParams.has("screensharevideoonly") ||
    urlParams.has("ssvideoonly") ||
    urlParams.has("ssvo")
  ) {
  ...
  if (urlParams.has("screensharefps") || urlParams.has("ssfps")) {
    if (urlParams.get("screensharefps") || urlParams.get("ssfps")) {
      session.screensharefps =
        urlParams.get("screensharefps") || urlParams.get("ssfps");
  ...
  if (urlParams.has("screensharequality") || urlParams.has("ssq")) {
    if (urlParams.get("screensharequality") || urlParams.get("ssq")) {
      session.screensharequality =
        urlParams.get("screensharequality") || urlParams.get("ssq");
  ...
  if (urlParams.has("screensharebitrate") || urlParams.has("ssbitrate")) {
    session.screenShareBitrate =
      urlParams.get("screensharebitrate") || urlParams.get("ssbitrate");
  ...
  if (urlParams.has("screensharelabel") || urlParams.has("sslabel")) {
    session.screenShareLabel =
      urlParams.get("screensharelabel") || urlParams.get("sslabel");
  ...
  if (urlParams.has("whepshare") || urlParams.has("whepsrc")) {
    ...
    try {
      session.whepSrc =
        urlParams.get("whepshare") || urlParams.get("whepsrc") || null;
  ...
  if (session.roomid !== false) {
    ...
    if (session.webcamonly == true) {
      ...
      getById("head1").innerHTML =
        '<span style="color:#CCC" data-translate="please-select-option-to-join">Please select an option to join.</span>';
    }
    ...
    if (session.roomid.length > 0) {
      if (session.videoDevice === 0) {
        ...
        miniTranslate(
          getById("add_microphone"),
          "join-room-with-mic-only",
          "Join Room with just Microphone"
        );
      ...
      miniTranslate(
        getById("add_screen"),
        "share-screen-with-room",
        "Screenshare with Room"
      );
    } else {
      if (session.videoDevice === 0) {
        miniTranslate(
    ...
    if (directorLanding == false) {
      ...
      try {
        ...
        if (
          urlParams.has("codirector") ||
          urlParams.has("directorpassword") ||
          urlParams.has("dirpass") ||
          urlParams.has("dp")
        ) {
          session.directorPassword =
            urlParams.get("codirector") ||
            urlParams.get("directorpassword") ||
            urlParams.get("dirpass") ||
            urlParams.get("dp");
          if (!session.directorPassword) {
            window.focus();
            session.directorPassword = await promptAlt(
              getTranslation("enter-director-password"),
              true
            );
          } else {
            session.directorPassword = decodeURIComponent(
              session.directorPassword
            );
          }
          if (session.directorPassword) {
            session.directorPassword = sanitizePassword(
              session.directorPassword
            );
            await generateHash(
              session.directorPassword + session.salt + "abc123",
              12
            )
              .then(function (hash) {
                // million to one error.
  ...
  if (
    urlParams.has("nofileshare") ||
    urlParams.has("nodownloads") ||
    urlParams.has("nofiles")
  ) {
  ...
  if (
    (session.view && session.roomid === false) ||
    (session.waitImage && session.scene !== false)
  ) {
    getById("container-4").className = "column columnfade";
    getById("container-3").className = "column columnfade";
    getById("container-2").className = "column columnfade";
          ...
          if (session.view) {
            if (document.getElementById("mainmenu")) {
              if (session.waitImage) {
                ...
                getById("retryimage").src = decodeURIComponent(
                  session.waitImage
                );
                ...
                getById("retrySpinner").title = getTranslation(
              ...
              if (urlParams.has("waitmessage")) {
                ...
                getById("retrymessage").innerText =
                  urlParams.get("waitmessage");
    ...
    if (
      ...
      if (!session.cleanOutput) {
        warnUser(
          "Safari requires us to ask for an audio permission to use peer-to-peer technology. You will need to accept it in a moment if asked to view this live video",
          20000
        );
      }
  ...
  if (location.protocol !== "https:") {
    if (!session.cleanOutput) {
      warnUser(
        "SSL (https) is not enabled. This site will not work without it!<br /><br /><a href='https://" +
          window.location.host +
          window.location.pathname +
          window.location.search +
          "'>Try accessing the site from here instead.</a>",
        false,
        false
      );
    }
  ...
  session.remoteInterfaceAPI = function (e) {
    ...
    if ("groups" in e.data) {
      ...
      eleGroup
        .querySelectorAll('[data-action-type="toggle-group"][data-group]')
        .forEach((group) => {
      ...
      if (session.group) {
        session.group.forEach((group) => {
          var ele = eleGroup.querySelector(
            '[data-action-type="toggle-group"][data-group="' + group + '"'
          );
          if (!ele) {
      ...
      for (var i in session.rpcs) {
        try {
          ...
          if ("streamID" in session.rpcs[i]) {
            if ("target" in e.data) {
              if (
                session.rpcs[i].streamID == e.data.target ||
                e.data.target == "*"
              ) {
          ...
          if ("streamID" in session.rpcs[i]) {
            ...
            if ("target" in e.data) {
              if (
                ...
                // specify a stream ID or let it apply to all videos
                session.requestAudioRateLimit(
                  parseInt(e.data.bitrate),
                  i,
                  lock
                );
              }
                  ...
                  if (stat.type == "outbound-rtp") {
                    if (stat.kind == "video") {
                      ...
                      if ("encoderImplementation" in stat) {
                        session.pcs[UUID].stats.encoder =
                    ...
                    if ("totalRoundTripTime" in stat) {
                      if ("responsesReceived" in stat) {
                        session.pcs[UUID].stats.average_roundTripTime_ms =
                          parseInt(
                            (stat.totalRoundTripTime / stat.responsesReceived) *
                              1000
                          );
                      }
                    }
    ...
    if ("requestStatsContinuous" in e.data) {
      if (session.remote) {
        session.sendRequest({
          requestStatsContinuous: e.data.requestStatsContinuous,
    ...
    if ("getLoudness" in e.data) {
      ...
      if (e.data.getLoudness == true) {
        ...
        var loudness = {};
        for (var i in session.rpcs) {
          loudness[session.rpcs[i].streamID] =
            session.rpcs[i].stats.Audio_Loudness;
        }
          ...
          if ("streamID" in session.rpcs[i]) {
            if (
              ...
              try {
                if ("settings" in e.data) {
                  try {
                    for (const property in e.data.settings) {
                      try {
                        session.rpcs[i].videoElement[property] =
                          e.data.settings[property];
                      } catch (e) {}
                    }
                  } catch (e) {}
                }
                if ("add" in e.data) {
                  try {
                    getById("gridlayout").appendChild(
                  ...
                  try {
                    session.rpcs[i].videoElement.parentNode.removeChild(
                      session.rpcs[i].videoElement
                    );
                  } catch (e) {
                    try {
                      session.rpcs[
                        i
                      ].videoElement.parentNode.parentNode.removeChild(
                        session.rpcs[i].videoElement.parentNode
                      );
                    } catch (e) {}
                  }
                  ...
                  try {
                    ...
                    getById("gridlayout").childNodes.forEach((ele) => {
                      if (
                        !ele.id ||
                        ele.id !== session.rpcs[i].videoElement.id
                      ) {
  ...
  if (timezones.includes(currentTimezone)) {
    var el = getById("languagesList").querySelector(
      "li a[data-tz*='" + currentTimezone + "']"
    ); // select language li
    el.parentElement.removeChild(el); // remove it
    getById("languagesList").insertBefore(
      el,
      getById("languagesList").querySelector("li:nth-child(2)")
    ); // insert it after English
  }
  ...
  function keyDownEvent(event) {
    ...
    if (event.key === "Escape") {
      ...
      if (document.fullscreenElement) {
        ...
        let userWarnings = document.querySelectorAll(
    ...
    if (PPTHotkey) {
      if (event.target && event.target.tagName == "INPUT") {
        ...
        PPTHotkey.ctrl === event.ctrlKey &&
        PPTHotkey.alt === AltPressed &&
        PPTHotkey.meta === event.metaKey &&
        (PPTHotkey.key === false ||
          (PPTHotkey.key !== false && PPTHotkey.key === event.key))
      ) {
  ...
  document.addEventListener("keyup", (event) => {
    ...
    if (
      KeyPressedTimeout &&
      (event.keyCode == 77 || !(event.ctrlKey || event.metaKey))
    ) {
